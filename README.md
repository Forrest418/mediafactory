...
1. This is a complete Visual Studio project. Developers can open `MediaFactory.slnx` in Visual Studio, build the project, and run the application.
2. If you are not a developer, download the published release package and run the program directly.

### Requirements

- Windows x64
- `ModelProviders.json` configured for your own model providers
- Internet access for model requests
- For developers: Visual Studio with .NET 8 SDK
- For release packages: the published `win-x64` build is self-contained and does not require a separate .NET runtime installation
...
1. 这是一个完整的 Visual Studio 项目，开发者可以直接用 Visual Studio 打开 `MediaFactory.slnx`，生成并运行程序。
2. 如果不是开发者，直接下载发布的程序使用即可。

### 程序依赖

- Windows x64
- 需要按自己的环境配置 `ModelProviders.json`
- 需要联网访问模型服务
- 开发者模式需要 Visual Studio 和 .NET 8 SDK
- 发布包为 `win-x64` 自包含版本，普通使用者无需单独安装 .NET 运行时
